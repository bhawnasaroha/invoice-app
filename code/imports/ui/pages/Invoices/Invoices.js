...
const Invoices = ({ loading, invoices, match, history }) => {
return (!loading ? (
    ...
    {invoices.length ? <Table hover responsive>
      ...
      <tbody>
        {invoices.map(({ _id, number, status, createdAt, client, subject, total }) => (
          <tr key={_id}>                    
            <td onClick={() => history.push(`${match.url}/${_id}`)} className="text-center">{getInvoiceLabel(status)}</td>
            ...
            <td>              
              <button onClick={() => history.push(`${match.url}/${_id}/edit`)} className="btn btn-primary editButton" value="edit">
