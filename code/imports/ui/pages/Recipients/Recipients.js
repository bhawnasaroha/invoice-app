    ...
    {recipients.length ? <Table hover responsive>
      ...
      <tbody>
        {recipients.map(({ _id, name }) => (
          <tr key={_id}  >
            <td onClick={() => history.push(`${match.url}/${_id}`)}>{name}</td>
            <button onClick={() => history.push(`${match.url}/${_id}/edit`)} className="btn btn-primary editButton" value="edit">Edit</button>
          </tr>
        ))}
      </tbody>
